...
def create_tracker(tracker_type, tracker_config, reid_weights, device, half, per_class, conf_gun, overlap_gun):
    ...
    if tracker_type == 'strongsort':
    ...
    elif tracker_type == 'deepocsort_confirm':
        print('use gun detection-based confirmation')
        from boxmot.trackers.deepocsort_confirm.deep_ocsort_confirm import DeepOCSort
        ...
        deepocsort = DeepOCSort(
            ...
            asso_func=cfg.asso_func,
            inertia=cfg.inertia,
            conf_gun=conf_gun, # change to just conf_gun to use input args instead of config
            overlap_gun=overlap_gun, # change to just overlap_gun to use input args instead of config
            cmc_off=cfg.cmc_off
        )
        return deepocsort
    else:
