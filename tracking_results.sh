...
for model_weight in $model_weights_folder/*; do
    ...
    # Iterate through conf_gun values
    for conf_gun in $(seq $min_value $step $max_value); do
        # Iterate through overlap_gun values
        for conf in $(seq $min_value $step $max_value); do
            echo "Running experiments with conf_gun=$conf_gun and conf=$conf in parallel"

            experiment_name="exp_${model_name}_conf_gun${conf_gun}_conf${conf}"
            ...
            for video in "${videos[@]}"; do
                ...
                nohup python examples/track.py --save-mot --yolo-model "$model_weight" --source "inference/videos/${video}" --name "exp_${model_name}_video${video%.*}_conf_gun${conf_gun}_conf${conf}" --device 0 --conf_gun "$conf_gun" --conf "$conf" --project Tracking_Outputs --tracking-method deepocsort_confirm &
            done
            ...
            for experiment_folder in "Tracking_Outputs"/*; do
                
                # Create the destination directory
                mkdir -p "MOT-ASTERS/test/${experiment_name}/"
                ...
                for video in "${videos[@]}"; do
                    ...
                    if [ -e "$txt_file" ]; then
                        mv "$txt_file" "MOT-ASTERS/test/${experiment_name}/"
                    else
                        touch "MOT-ASTERS/test/${experiment_name}/${video%.*}.txt"
                    fi
                done
            done
            ...
            rm -rf Tracking_Outputs/*
            
            # Compute mot metrics and save console as a txt file
            mkdir -p "${output_folder}/${experiment_name}/"
            python -m motmetrics.apps.eval_motchallenge MOT-ASTERS/train/ "MOT-ASTERS/test/${experiment_name}" > "${output_folder}/${experiment_name}/metrics.txt"
        done
    done
done